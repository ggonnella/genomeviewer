...
class FormatTest < ActiveSupport::TestCase
  ...
  def test_attribute_lists
    ...
    all = Format::StyleTypes.map{|t| Format.send("list_#{t}")}.flatten
    assert_equal all.to_set, Format.style_attributes.to_set
  end

  def test_show_grid
    args = ["format","show_grid"]
    assert_equal @style.gt.get_bool(*args), @format.default_show_grid
    assert_not_equal false, @format.show_grid
    assert_not_equal false, @style.gt.get_bool(*args)
    @format.sync_show_grid = false
    assert_equal false, @format.show_grid
    assert_equal false, @style.gt.get_bool(*args)
  ...
  Format.list_floats.each do |f|
    ...
    args = ["format",f.to_s]
    assert_equal @style.gt.get_num(*args), @format.send("default_#{f}")
    assert_not_equal @a_float, @format.send(f)
    assert_not_equal @a_float, @style.gt.get_num(*args)
    @format.send("sync_#{f}=", @a_float)
    assert_in_delta @a_float, @format.send(f), 0.000000000001
    assert_equal @a_float, @style.gt.get_num(*args)
    end
  end
  ...
  Format.list_colors.each do |col|
    ...
    assert_equal Color(@style.gt.get_color(*args)),
